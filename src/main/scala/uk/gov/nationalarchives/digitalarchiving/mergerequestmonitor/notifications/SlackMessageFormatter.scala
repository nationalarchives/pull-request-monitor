...
object SlackMessageFormatter {
  ...
  def format(projects: Seq[ProjectSlackPresenter], mergeRequestsTerm: MergeRequestsTerm): String = {
    ...
    if (projectsWithOpenMrs.isEmpty) {
      ...
      val oldPrProjectMessages: String = projectsToString(
        projectsWithOpenMrs,
        pr => pr.daysSinceLastUpdate >= 2,
        mergeRequest => s"${newPrToString(mergeRequest)} ${updatedSince(mergeRequest.daysSinceLastUpdate)}"
      )

      val welcomeMsg = "Hello team!\n"
      val oldPrs =
        if (oldPrProjectMessages.nonEmpty) s"\n${welcomeMsg}These pull requests have had no activity for more than two days:\n$oldPrProjectMessages\n\n" else welcomeMsg
      if (newPrProjectMessages.nonEmpty) s"${oldPrs}Here are the pull requests to review today:\n$newPrProjectMessages\n" else oldPrs
    }
  }

  private def projectsToString(
      projectsWithOpenMrs: Seq[ProjectSlackPresenter],
      filterFn: MergeRequestSlackPresenter => Boolean,
      strFn: MergeRequestSlackPresenter => String
  ) = {
    projectsWithOpenMrs
      .map { pr =>
