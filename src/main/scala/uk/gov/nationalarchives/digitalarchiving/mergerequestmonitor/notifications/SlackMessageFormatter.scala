...
object SlackMessageFormatter {
  ...
  def format(projects: Seq[ProjectSlackPresenter], mergeRequestsTerm: MergeRequestsTerm): String = {
    ...
    if (projectsWithOpenMrs.isEmpty) {
      ...
      def newPrToString: MergeRequestSlackPresenter => String = mergeRequest => s"         ${mergeRequest.authorName}: <${mergeRequest.url}| ${mergeRequest.title}>${mergeRequest.draft} ${mergeRequest.reviewStatus}"
